...
//    try
//    {
//        Mavlink_Control mavconn;
//        int result = mavconn.start(argc,argv);
//        return result;
//    }
//
//    catch ( int error )
...
Mavlink_Control::Mavlink_Control(int baudrate, char *&uart_name)
{
    location_manager = Location_Manager(&syslog);

    //   PORT and THREAD STARTUP
    serial_port = Serial_Port(uart_name, baudrate);

    autopilot_interface = new Autopilot_Interface(&serial_port, &syslog, &location_manager);
...
void Mavlink_Control::parse_commandline(int argc, char **argv, char *&uart_name, int &baudrate)
{
...
void Mavlink_Control::takeoff(float  h, mavlink_set_position_target_local_ned_t &sp)
{
    ...
    while (autopilot_interface->current_messages.local_position_ned.z > h)
    {
        ...
        syslog.write2txt("current takeoff position ", autopilot_interface->current_messages.local_position_ned);
        autopilot_interface->update_setpoint(sp);
        usleep(50000);
    }
...
void Mavlink_Control::land(mavlink_set_position_target_local_ned_t &sp)
{
    ...
    {
        //printf("current height is %f \n", (float)autopilot_interface->current_messages.local_position_ned.z);
        syslog.write2txt("current land position ", autopilot_interface->current_messages.local_position_ned);
        usleep(50000);
    }
...
void Mavlink_Control::goto_local_position(float x, float y, float z, mavlink_set_position_target_local_ned_t &sp)
{
    ...
    while (!IsInWaypointLocal(autopilot_interface->current_messages.local_position_ned, sp, 0.5))
    {
        ...
        syslog.write2txt("current position (after command to move) ", autopilot_interface->current_messages.local_position_ned);
...
void Mavlink_Control::goto_global_position(float lat, float lon, float alt, mavlink_set_position_target_local_ned_t &sp)
{
    ...
    Location_Manager locman(&syslog);
