...
#include "mono_offline_viorb.h"


Mono_Offline_VIORB::Mono_Offline_VIORB(boost::mutex *pMu)
{
    mu = pMu;
    time_to_exit = false;
    isFirstFrame = false;
...
void Mono_Offline_VIORB::start(char *&vocabulary, char *&setting) {
    ...
    //boost::thread threadSLAM = boost::thread(&Mono_Offline_VIORB::grabFrameData, this);
    grabFrameData();
